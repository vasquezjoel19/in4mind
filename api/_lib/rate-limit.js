...
/* Las credenciales salen de `request-auth.js` y no del entorno directamente:
   ese módulo ya lleva un respaldo para cuando las variables no están, y
   leerlas aquí por separado desactivaría la cuota en silencio justo en los
   despliegues donde la autenticación sí funciona. */
const { SUPABASE_URL, SUPABASE_ANON_KEY } = require('./request-auth.js');
...
function isQuotaConfigured() {
  return Boolean(perDayLimit() && SUPABASE_URL && SUPABASE_ANON_KEY);
}
